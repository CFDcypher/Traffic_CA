...
cdef bint CL_safety_criterion(int vel, int g_b_AL, int v_b_AL, str model):
    ...
    if (model == 'S-NFS') or (model == 'KKW'):
        ...
        return g_b_AL >= 1 #vel >= v_b_AL - g_b_AL + W184_gap
...
cdef change_lane(int[:,:] cars_list, int[:,:] sites, int n_hdv, double P_lc, int num_change_lane, str model):
    ...
    for i in range(num_cars):
        if (r[i] < P_lc or i >= n_hdv):
            ...
            if CL_incentive_criterion(vel[i], g[i,pres_lane,0,0], v[i,pres_lane,0,0], g[i,adj_lane,0,0], v[i,adj_lane,0,0], t[i,pres_lane,0,0], t[i,adj_lane,0,0], model, agent_type(i, n_hdv, num_cars - n_hdv)):
...
cdef int[:] vel_step_W184(int[:,:] cars_list, int[:,:] g, int n_hdv, int max_platoon_size, bint red_light):
    cdef int num_cars = len(cars_list)
...
cdef move_W184(int[:,:] cars_list, int[:,:] sites, int n_hdv, int max_platoon_size, int N_c, double P_lc, int num_change_lane, bint red_light):
    ...
    v = vel_step_W184(cars_list, g, n_hdv, max_platoon_size, red_light)
